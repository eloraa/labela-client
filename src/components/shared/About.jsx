        ...
        <div className="mt-28 float-left w-full lg:w-[calc(100%-452px)] max-md:order-2">
          ...
          <form onSubmit={handleSubmit} className="w-full relative">
            <div className="w-full">
              <input
                className="w-full py-4 dark:bg-transparent outline-none border-b-2 dark:valid:border-dark dark:focus:border-dark valid:border-black focus:border-black"
                type="email"
                name="email"
                placeholder="Email"
                required
            ...
            <div className="w-full mt-6">
              <button className="bg-black dark:bg-dark dark:text-black w-full max-md:mt-6 py-2.5 md:px-24 px-0 text-white font-bold rounded active:scale-[.99] transition-transform text-sm">
                Subscribe
              </button>
            </div>
            {isSubbed && <div className="absolute inset-0 bg-white border-2 dark:border-black dark:bg-black/60 flex justify-center items-center font-bold">Thanks for Subscribing.</div>}
          </form>
        </div>
      </div>
    </div>
